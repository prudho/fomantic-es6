...
const settings = {
    ...
    events: [
        'Reposition', // Called when position is recalculated
        'Scroll',     // Called on each scroll
        'Stick',      // Called when element is stuck to viewport
        'Unstick',    // Called when element is unstuck from viewport
        'Top',        // Called when element reaches top of context
        'Bottom',     // Called when element reaches bottom of context
    ]
}
...
export class Sticky extends Module {
    ...
    initialize() {
        ...
        this.verbose('Initializing rating module', this.settings, this.$container);
    ...
    save_positions() {
        ...
        if (!this.is_standardScroll()) {
            ...
            scrollContext.top  = this.$scroll.scrollTop();
            scrollContext.left = this.$scroll.scrollLeft();
    ...
    stick(scroll) {
        var
            cachedPosition = scroll || this.$scroll[0].scrollY,
            cache          = this.cache,
            fits           = cache.fits,
            ...
            direction      = this.get_direction(scroll.top),
            elementScroll  = (fits)
                ? 0
        ...
        if (elementVisible && !sameHeight) {
            if (this.is_initialPosition()) {
                ...
                } else if (this.is_bottom() ) { // currently fixed bottom
                    // top edge
    ...
    fixTop() {
        ...
        ;
        this.invokeCallback('Stick').call(this.element);
    }
    ...
    bindTop() {
        ...
        this.$element
            ...
            .addClass(this.settings.className.top)
        ;
        this.invokeCallback('Top').call(this.element);
        this.invokeCallback('Untick').call(this.element);
    }
    ...
    bindBottom() {
        ...
        this.$element
            ...
            .addClass(this.settings.className.bottom)
        ;
        this.invokeCallback('Bottom').call(this.element);
        this.invokeCallback('Unstick').call(this.element);
    }
    ...
    refresh(hardRefresh) {
        ...
        this.save_positions();
        this.stick();
        this.invokeCallback('Reposition').call(this.element);
    }
    ...
    event_scroll() {
        requestAnimationFrame(function() {
            ...
            this.$scroll.trigger('scrollchange' + this.eventNamespace, this.$scroll[0].scrollY );
        }.bind(this));
    }

    event_scrollchange(event, scrollPosition) {
        this.stick(scrollPosition);
        this.invokeCallback('Scroll').call(this.element);
    }
    ...
    is_hidden() {
        //return (!this.$element.is(':visible')); INVESTIGATE
        return false;
    }
    ...
    get_scrollChange(scroll) {
        scroll = scroll || this.$scroll.scrollY;
        return (this.lastScroll) ? (scroll - this.lastScroll) : 0;
    }

    set_containerSize() {
        /* INVESTIGATE
        var tagName = this.$container.get(0).tagName;
    ...
    set_minimumSize() {
        this.$container.css('min-height', this.cache.height);
    }
